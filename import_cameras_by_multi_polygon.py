...
class ImportCameraDlg(QtWidgets.QDialog):
    ...
    def selectShapeFile(self):
        ...
        self.shapesFile = QtWidgets.QFileDialog.getOpenFileNames(
            self, 'Open File', 'c\\', 'Image files (*.shp)')
        self.shpFiles = self.shapesFile[0] 
    ...
    def getWKTshp(self , shpFiles):
         self.totalShp = len( self.shpFiles)
         for i in range(self.totalShp):
            shp = self.shapesFile[0][i]
...
label = "Custom menu/Import Cameras MultiShape"
