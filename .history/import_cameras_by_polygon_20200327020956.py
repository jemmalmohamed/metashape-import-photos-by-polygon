...
class ImportCameraDlg(QtWidgets.QDialog):
    ...
    def selectFolder(self):
        ...
        self.shapeFile = Metashape.Shape
        ...
        for (root, dirs, files) in os.walk(directoryPath):
            ...
            for s in chunk.shapes:
                ...
                self.shapeFile.vertices = [Metashape.CoordinateSystem.transform(
                    v, source, lambert) for v in self.shapeFile.vertices]
                # print(shape.vertices)
                poly = geometry.Polygon([[p.x, p.y]
                                         for p in self.shapeFile.vertices])
                print(poly.wkt)
                # for file in files:
